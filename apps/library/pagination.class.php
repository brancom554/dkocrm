...
class Pagination {
    ...
    function getEndLimit($currentPage) {
        ...
        if ($this->lastItem > $this->totalRecord)
            $this->lastItem = $this->totalRecord;
    ...
    function paginate($url, $currentPage) {
        if ($this->totalPage > 1) {
            ...
            if ((int) $currentPage > 1) {
                echo '<a class="previous page-numbers" href="/' . $url . '/p_' . ($currentPage - 1) . '"><i class="bx bx-chevron-left"></i></a>';
            }

            for ($p = $startRange; $p <= $endRange; $p++) {
                if ($currentPage == $p) {
                    echo '<span class="page-numbers current" aria-current="page">' . $p . '</span>';
                } else {
                    echo '<a class="previous page-numbers" href="/' . $url . '/p_' . $p . '">' . $p . '</a>';
                }
            }
            if ($currentPage < $this->totalPage) {
                echo '<a class="previous page-numbers" href="/' . $url . '/p_' . ($currentPage + 1) . '"><i class="bx bx-chevron-right"></i></a>';
            }
        }
    }
