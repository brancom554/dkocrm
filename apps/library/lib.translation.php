...
class Translation {
    ...
    function __construct() {
        ...
        if (($this->lang != 'en' && $this->lang != 'fr' && $this->lang != 'pt' && $this->lang != 'de') || !trim($this->lang))
            $this->lang = 'fr';
    ...
    protected function loadCsvData($filename, $locale, $separator) {
        ...
        while (!feof($this->_file)) {
            ...
            if ((!is_array($content) and ( substr(trim($content), 0, 1) == "#")) or ( is_array($content) and ( substr(trim($content[0]), 0, 1) == "#"))) {
                continue;
            }
            if (!empty($content[1])) {
    ...
    function trl($input) {
        if (isset($this->data[$this->lang][$input]))
            return($this->data[$this->lang][$input]);
        else
            return($input);
    }
