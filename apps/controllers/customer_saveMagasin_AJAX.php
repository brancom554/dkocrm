...
if (
	...
	if (empty(trim($civilite))) {
		...
		echo "false||Votre numéro de téléphone est incorrect, mettez que des chiffres.";
		exit;
	} elseif (!preg_match('/^[0-9_]+$/', $business_phone_number)) {
		echo "false||Le numéro de téléphone de votre magasin est incorrect, , mettez que des chiffres.";
		exit;
		...
		//check if user email exist before continue ! 
			...
			$register_business = $sqlData->insertNewBusiness($magasin_code, $business_name, $nom_user, $business_address_1, $business_address_2, $zipcode, $business_city, $business_phone_number, $business_email, $prenom_user, $business_email, $magasin_url);
			// exit;
			if ($register_business) {
				$register_user = $sqlData->insertNewUser($contact_num, $civilite, $nom_user, $prenom_user, $adresse1, $adresse2, $zipcode, $ville, $phone_number, $email, $magasin_code, $profile_id);
